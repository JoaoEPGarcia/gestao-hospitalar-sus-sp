...
| Eficiência produtiva | não estimada no relatório | não se aplica | OSS com log lambda 0,536 menor (cerca de 41% menos distância à fronteira), IC exclui zero, estável nas três variantes | ordem confirmada (OSS mais perto da fronteira); NÍVEL não citável, pois a TE das OSS é constante por degeneração; Spearman DEA vs SFA 0,625 (0,648 excluídas as OSS) | converge em direção entre os dois métodos; magnitude interpretável só no DEA |
