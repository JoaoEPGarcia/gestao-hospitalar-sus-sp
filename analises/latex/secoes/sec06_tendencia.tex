...
O custo pede uma leitura em duas camadas. Em valores correntes
...
valores de 2025, o custo mediano caiu de R\$~1.496 para R\$~1.317, uma
queda real de cerca de 12\% na década}. Toda a alta aparente é
inflação. Qualquer avaliação da evolução de custo deve, portanto, ser
...
forma consistente na segunda metade do período; e, no custo, a Direta
parte de nível parecido com as demais categorias e passa a liderar o
custo mediano a partir de 2020.

\begin{table}[H]
\centering
\caption{Custo por saída mediano por ano: em valores correntes e em
...
\begin{figure}[H]
\centering
\subfig{fig_ae_07_tendencia_custo_saida.png}{Custo por saída (valores
correntes)}
\subfig{fig_ae_07_tendencia_pct_alta_complex.png}{Fração alta
complexidade}
\caption{Mediana anual do custo por saída (valores correntes) e da
fração de alta complexidade.}
\label{fig:tendB}
\end{figure}

\comoler{o custo em valores correntes sobe a década inteira, mas essa
...
\caption{Custo por saída mediano: a linha clara tracejada é o valor
...
\begin{figure}[H]
\centering
\subfig{fig_ae_08_categoria_custo_saida.png}{Custo por saída (Privado
omitido por escala)}
\subfig{fig_ae_08_categoria_pct_alta_complex.png}{Fração alta
complexidade (Privado omitido por escala)}
\caption{Mediana anual do custo por saída e da fração de alta
complexidade por categoria. O Privado foi omitido destes dois painéis
porque sua escala (custo dez vezes maior) esconderia as demais
categorias; seus valores estão na Tabela~\ref{tab:categorias}.}
\label{fig:tendcatB}
\end{figure}

\comoler{no custo, as quatro categorias sobem juntas até 2019 e se
